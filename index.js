...
class MatrixRain {
    ...
    if (row >= 0 && row < this.numRows && col >= 0 && col < this.numCols) {
      ...
      if (this.mask) {
        ...
        if (
          maskRow >= 0 &&
          maskCol >= 0 &&
          maskRow < this.maskHeight &&
          maskCol < this.maskWidth &&
          this.mask[maskRow] &&
          this.mask[maskRow][maskCol] === this.maskBlankChar
        ) {
          str = ' ';
        }
      }
  ...
  renderFrame() {
    const ansiColor =
      ansi.colors[
        'fg${this.color.charAt(0).toUpperCase()}${this.color.substr(1)}'
      ]();
